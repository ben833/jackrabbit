...
const queue = (options) => {
    ...
    const consume = (callback, consumeOptions) => {
        ...
        const onMessage = (msg) => {
            ...
            console.log('TEMP: in onMessage, after parseMessage: data: ', data);
            if (!data) {
                return;
            }
        ...
        if ( ready ){
            ...
            console.log(`TEMP: in if ( ready ): about to call channel.consume with emitter.name '${emitter.name}'
                and opts: `, opts);
        ...
        emitter.once('ready', () => {
            ...
            const opts = Extend({}, DEFAULT_CONSUME_OPTIONS, consumeOptions);
            console.log(`TEMP: in emitter.once('ready'): about to call channel.consume with emitter.name '${emitter.name}'
                and opts: `, opts);
            channel.consume(emitter.name, onMessage, opts, onConsume);
        });
    };
    ...
    const parseMessage = (msg) => {
        ...
        console.log('TEMP: in parseMessage, about to check for properties: msg: ', msg);
